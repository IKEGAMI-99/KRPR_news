(() => {
  ...
  const DEV_CACHE_KEY = 'kirapara-dev-status-v5';
    ...
    <div class="developer-panel" hidden>
      <div class="dev-topline">
        <div>
          ...
          <div class="dev-model">Gemma 4 E4B · LiteRT-LM</div>
        </div>
      ...
      <div class="dev-status-card dev-loading">
        ...
        <div><strong>取得中…</strong><small>Gemma 4 workflowを確認しています</small></div>
      </div>
      <div class="dev-metrics"></div>
      <div class="dev-steps"></div>
      <div class="dev-runs"></div>
      <a class="dev-actions-link" href="https://github.com/IKEGAMI-99/KRPR_news/actions/workflows/ai-translate.yml" target="_blank" rel="noopener noreferrer">Gemma 4 Actionsで詳細を見る ↗</a>
    </div>`;
  ...
  function renderStatus(data) {
    ...
    const planStep = stepByName(job, 'Recheck strict Japanese pending translations') || stepByName(job, 'Count pending translations');
    ...
    card.innerHTML = `<span class="dev-status-dot"></span><div><strong>AI ${mainLabel}</strong><small>${aiStep?.name || job?.name || 'Translate News with Gemma 4 E4B LiteRT'}${aiStep?.started_at ? ` · ${elapsed(aiStep.started_at, aiStep.completed_at)}` : ''}</small></div>`;
