...
外部Feedが一時的に空になった、取得先がタイムアウトした、といった障害を「公式が投稿を削除した」と誤認すると、大量の記事が一度に消える危険があります。そのため、公式Xの履歴保護や地域単位の直前データ保持など、**last known good（最後に確認できた正常値）を優先するフェイルセーフ**を採用しています。
