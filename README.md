...
中国語記事では、原文にも存在する一般語「活动」と数値付き割引表記を検証前に補正します。`5.1折 → 49%OFF`、`7.8折 → 22%OFF` のように十進数で正確に換算し、URL・ハッシュタグ・用語集の保持指定名は変更しません。補正後も通常の日本語品質チェックを行います。

準備処理は未完了総数 `pending`、実行可能数 `runnable`、再試行待ち数 `deferred` を別々に出力します。Workflowは `runnable` を見てモデル準備・翻訳の必要性を判断し、待機中の記事を翻訳済み扱いにはしません。監査による個別補正は原文ハッシュに紐付けられ、原文が変われば失効して再翻訳の対象になります。
