...
外部サーバーが接続だけ維持して応答を完了しない場合にも毎時更新全体が待たされないよう、各収集・画像処理には処理単位のハードタイムアウトを設けています。任意ソースが制限時間を超えた場合は、その処理を止めて実行前のニュース・日時・画像品質データへロールバックし、直前の正常値を使って後続処理を継続します。Workflow全体にも15分の上限があり、最終成功から70分以上更新されなければwatchdogが再実行します。
...
Weibo画像は外部CDN直リンクの表示が不安定なため `docs/media/weibo/` へミラーし、PWAではGitHub Pages同一オリジンの画像を優先します。Xでは元投稿側の高解像度候補を優先し、TapTapでは記事本文・投稿メディアに属する画像を優先します。BilibiliではBFS CDNの `@316w_560h_...` のような縮小・クロップ派生URLを原寸URLへ戻し、`i0` / `i1` / `i2` のCDN別名も同じ画像として重複除去します。
...
ニュース収集は毎時00分に開始します。GitHub Actionsの混雑や取得先の応答時間により実際の完了時刻は数分ずれることがあります。各外部処理は `scripts/run_refresh_pipeline.py` が個別のハードタイムアウトとデータスナップショットを管理し、任意ソースの遅延だけで全体が止まらないようにしています。ヘッダーの「最終更新」は記事の公開日時ではなく、`news-refresh.yml` が収集・画像処理まで正常に完了して `data/crawl_status.json` を更新した時刻です。新着記事が0件でもクロール自体が成功すればこの時刻は更新されます。
...
- タイムラインは地域ごとに20件単位でページングし、表示中の20件だけをDOMへ生成
- 地域とページ番号はURLへ保持し、戻る・進む操作で同じ表示へ復元
- 記事更新で総ページ数が増減しても自動再計算し、存在しなくなったページは最後の有効ページへ補正
...
- 更新watchdog: 毎時 :17 / :47と翻訳完了時に確認し、最終成功から70分以上なら再起動

## ワークフロー

| Workflow | 実行 | 役割 |
| --- | --- | --- |
| `news-refresh.yml` | 毎時00分 | 処理ごとのハードタイムアウトと失敗時ロールバック、ニュース収集、公式X / Bilibiliの複数Feed統合と履歴保護、同一記事の出典統合、画像・日時の正規化、Weibo画像ミラー、クロール完了時刻の記録 |
| `news-refresh-watchdog.yml` | 毎時17 / 47分、翻訳完了時 | 最終成功時刻を確認し、70分以上更新されていない場合だけ収集Workflowを再実行 |
